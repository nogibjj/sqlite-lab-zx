...
def create_data(conn, data):
    ...
    cursor = conn.cursor()
    cursor.execute("INSERT INTO GroceryDB (id, general_name, count_products, ingred_FPro, avg_FPro_products, avg_distance_root, ingred_normalization_term, semantic_tree_name, semantic_tree_node) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", data)
    conn.commit()
...
def update_data(conn, new_value, item_id):
    ...
    cursor = conn.cursor()
    cursor.execute("UPDATE GroceryDB SET count_products = ? WHERE id = ?", (new_value, item_id))
    conn.commit()
